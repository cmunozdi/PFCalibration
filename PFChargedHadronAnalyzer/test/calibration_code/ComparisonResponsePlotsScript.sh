...
find $DIR_2025/plots -type f -name "*.png" | while read -r png_2025; do
    ...
    if [[ -f "$root_2025" && -f "$root_2024" ]]; then
        run_better_plotter "$root_2025" "$root_2024" "$folder" "$xlog" "$title" "$xlabel" "$ylabel" "$output" "$ylim_min" "$ylim_max" "$mode"
    else
        echo "Archivo root faltante para $png_name: $root_2025 o $root_2024"
    fi
done
